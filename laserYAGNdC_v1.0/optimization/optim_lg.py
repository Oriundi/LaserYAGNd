...
def solve_equation(x00, t00, t11, gamma0, tau_r0, R0, Cg0, Ca0, Cg_eps0, x_out, tau):
    def equation(t, x):
        # Xiao-Bass speed equations
        return numpy.array([R0 - Cg0 * x[0] * x[2] - x[0]/tau_g,
                           Ca0 * x[1] * x[2] + (Na_total - x[1])/tau_a,
                           (x[0]*g - x[1]*a1 - (Na_total-x[1])*a2 - 2*gamma0)
...
def optimizelg(interval):
    ...
    for ii in range(0, numpy.size(interval[0])):
        ...
        x_out = x0
        tau = time_start
        lg = interval[0][ii]   # reflectivity of output mirror
        gamma_i = alpha_p * lg
        gamma = gamma_i + 0.5 * (gamma_1 + gamma_2)
        lr = n * (lg + la)
        tau_r = 2 * lr / c0
        ...
        while result[0] is not "Success" and calc_time <= max_calc_time:
            result = solve_equation(x0, time_start, calc_time, gamma, tau_r, R, Cg, Ca, Cg_eps, x_out, tau)
    ...
    plt.figure(2)
    plt.subplot(221)
    plt.xlabel("Active medium thickness, um")
