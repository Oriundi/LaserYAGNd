...
def solve_equation(x00, t00, t11, gamma0, x_out, tau):
    def equation(t, x):
...
def optimizeR2(interval):
    ...
    for ii in range(0, numpy.size(interval[0])):
        ...
        x_out = x0
        tau = time_start
        R2 = interval[0][ii]   # reflectivity of output mirror
        print(R2)
        ...
        while result[0] is not "Success" and calc_time <= max_calc_time:
            result = solve_equation(x0, time_start, calc_time, gamma, x_out, tau)
    ...
    plt.figure(2)
    plt.subplot(221)
    plt.xlabel("Output mirror transmission (R2)")
