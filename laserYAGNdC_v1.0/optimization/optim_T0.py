...
def solve_equation(x00, t00, t11, Na_total0, x_out, tau):
    def equation(t, x):
        # Xiao-Bass speed equations
        return numpy.array([R - Cg * x[0] * x[2] - x[0]/tau_g,
                           Ca * x[1] * x[2] + (Na_total0 - x[1])/tau_a,
                           (x[0]*g - x[1]*a1 - (Na_total0-x[1])*a2 - 2*gamma)
...
def optimizeT0(interval):
    ...
    for ii in range(0, numpy.size(interval[0])):
        tau = time_start
        T0 = interval[0][ii]   # reflectivity of output mirror
        ...
        while result[0] is not "Success" and calc_time <= max_calc_time:
            result = solve_equation(x0, time_start, calc_time, Na_total, x_out, tau)
    ...
    plt.figure(2)
    plt.subplot(221)
    plt.xlabel("Initial absorber transmission (T0)")
    plt.ylabel("Laser frequency, kHz")
    plt.plot(interval[0], freq)
    plt.subplot(222)
    plt.xlabel("Initial absorber transmission (T0)")
    try:
        plt.ylabel("Impulse width on level P/%f" % width_cut)
    except Exception:
